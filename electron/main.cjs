...
// Point the shell connector at the bundled low-integrity sandbox launcher (packaged builds).
try {
  if (app.isPackaged && process.platform === "win32" && !process.env.NYX_SANDBOX_EXE) {
    process.env.NYX_SANDBOX_EXE = path.join(process.resourcesPath, "Nyx.Sandbox.exe")
  }
} catch (e) {}
...
function createWindow(startPath) {
  ...
  win.webContents.on("did-finish-load", () => {
    const css = [
      ...
      "html{scrollbar-color:#2a2a35 transparent;scrollbar-width:thin}",
      "body{padding-top:38px !important}",
      "#nyx-drag{position:fixed;top:0;left:0;right:0;height:38px;-webkit-app-region:drag;z-index:2147483647;background:transparent}",
      "#nyx-drag button,#nyx-drag a,#nyx-drag input{-webkit-app-region:no-drag}"
    ].join("")
    win.webContents.insertCSS(css).catch(() => {})
    win.webContents.executeJavaScript("(function(){if(!document.getElementById('nyx-drag')){var d=document.createElement('div');d.id='nyx-drag';document.body.appendChild(d);}})();").catch(() => {})
  })

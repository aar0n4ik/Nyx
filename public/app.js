...
// ---------- store ----------
let DB = { chats: [], active: null } // real data loaded by Vault in bootVault()
...
function emptyState() {
  ...
  QUICK().forEach((b) => {
    ...
    btn.onclick = () => { if (b.action === "specs") runSpecs(); else if (b.action === "update") runUpdate(); else if (b.action === "snapshot") runSnapshotCreate(); else if (b.action === "restore") runRestorePick(); else sendText(b.send) }
    q.appendChild(btn)
  })
  wrap.appendChild(q)
  return wrap
...
;(async function bootVault(){
  try {
    if (!window.NyxVault) await new Promise(function(res){ var sc=document.createElement("script"); sc.src="/vault.js"; sc.onload=res; sc.onerror=res; document.head.appendChild(sc) })
    if (window.NyxVault) {
      var st = await window.NyxVault.boot()
      if (st.locked) { showUnlockGate(); return }
      DB = await window.NyxVault.loadChats()
    }
  } catch (e) {}
  if (!DB || !Array.isArray(DB.chats)) DB = { chats: [], active: null }
...
// ---------- Nyx Vault: unlock gate (passphrase) ----------
var VAULT_I18N = {
  en: { title: "Locked", desc: "Enter your passphrase to decrypt your chats on this device.", ph: "Passphrase", unlock: "Unlock", wrong: "Wrong passphrase — try again." },
  ru: { title: "Заблокировано", desc: "Введите пароль, чтобы расшифровать чаты на этом устройстве.", ph: "Пароль", unlock: "Разблокировать", wrong: "Неверный пароль — попробуйте ещё раз." },
  uk: { title: "Заблоковано", desc: "Введіть пароль, щоб розшифрувати чати на цьому пристрої.", ph: "Пароль", unlock: "Розблокувати", wrong: "Невірний пароль — спробуйте ще раз." },
};
function vtx(k){ var m = VAULT_I18N[LANG] || VAULT_I18N.en; return m[k] || VAULT_I18N.en[k]; }
function showUnlockGate(){
  if (document.getElementById("nyx-unlock")) return;
  var ov = document.createElement("div");
  ov.id = "nyx-unlock";
  ov.style.cssText = "position:fixed;inset:0;z-index:9999;display:grid;place-items:center;background:var(--bg,#0a0a0a);padding:20px";
  var card = document.createElement("div");
  card.style.cssText = "max-width:340px;width:100%;text-align:center";
  card.innerHTML = ''
    + '<h2 style="margin:0 0 6px;color:var(--text,#f4f4f5);font-size:20px">' + vtx("title") + '</h2>'
    + '<p style="margin:0 0 16px;color:var(--muted,#a6a6b0);font-size:14px;line-height:1.45">' + vtx("desc") + '</p>'
    + '<input id="nyx-pass" type="password" autocomplete="current-password" placeholder="' + vtx("ph") + '" style="width:100%;padding:11px 13px;border-radius:11px;border:1px solid var(--line,rgba(255,255,255,.12));background:var(--panel,#1c1d20);color:var(--text,#f4f4f5);font-size:15px;box-sizing:border-box"/>'
    + '<div id="nyx-pass-err" style="color:var(--bad,#f2726f);font-size:13px;min-height:18px;margin:8px 0"></div>'
    + '<button id="nyx-unlock-btn" style="width:100%;padding:11px;border-radius:11px;border:0;background:var(--text,#f4f4f5);color:var(--bg,#0a0a0a);font-weight:600;font-size:15px;cursor:pointer">' + vtx("unlock") + '</button>';
  ov.appendChild(card); document.body.appendChild(ov);
  var inp = card.querySelector("#nyx-pass"), err = card.querySelector("#nyx-pass-err"), btn = card.querySelector("#nyx-unlock-btn");
  setTimeout(function(){ inp.focus(); }, 50);
  async function tryUnlock(){
    err.textContent = "";
    try {
      await window.NyxVault.unlockWithPassphrase(inp.value);
      DB = await window.NyxVault.loadChats();
      if (!DB || !Array.isArray(DB.chats)) DB = { chats: [], active: null };
      ov.remove();
      if (!DB.chats.length) newChat(); else { render(); renderThread(); }
    } catch (e) { err.textContent = vtx("wrong"); inp.value = ""; inp.focus(); }
  }
...
// ---------- Nyx Snapshot: capture / restore UI ----------
var SNAP_I18N = {
  en: { qSnapT: "System snapshot", qSnapS: "Save a .nyx blueprint of this PC", qRestoreT: "Restore from snapshot", qRestoreS: "Load a .nyx file and rebuild",
        create: "Create snapshot", done: "Snapshot saved (.nyx downloaded).", bad: "Not a valid Nyx snapshot file.",
        plan: "Restore plan (nothing has run yet):", restore: "Restore now", restored: "Restore finished:" },
  ru: { qSnapT: "Снимок системы", qSnapS: "Сохранить .nyx-слепок этого ПК", qRestoreT: "Восстановить из снимка", qRestoreS: "Загрузить .nyx и восстановить",
        create: "Создать снимок", done: "Снимок сохранён (.nyx скачан).", bad: "Это не похоже на снимок Nyx.",
        plan: "План восстановления (пока ничего не запущено):", restore: "Восстановить сейчас", restored: "Восстановление завершено:" },
  uk: { qSnapT: "Знімок системи", qSnapS: "Зберегти .nyx-зліпок цього ПК", qRestoreT: "Відновити зі знімка", qRestoreS: "Завантажити .nyx і відновити",
        create: "Створити знімок", done: "Знімок збережено (.nyx завантажено).", bad: "Це не схоже на знімок Nyx.",
        plan: "План відновлення (поки нічого не запущено):", restore: "Відновити зараз", restored: "Відновлення завершено:" },
};
function snx(k){ var m = SNAP_I18N[LANG] || SNAP_I18N.en; return m[k] || SNAP_I18N.en[k]; }

async function runSnapshotCreate(){
  var c = active(); if (!c) return;
  pushMsg("user", { text: snx("create") }); maybeName(c, "snapshot device");
  var tp = showTyping();
  try {
    var r = await fetch("/api/snapshot").then(function(x){ return x.json(); }); tp.remove();
    if (!r || r.error) { pushMsg("bot", { text: ((r && r.error) || t("failed")) }); return; }
    var blob = new Blob([JSON.stringify(r.snapshot, null, 2)], { type: "application/json" });
    var url = URL.createObjectURL(blob);
    var a = document.createElement("a"); a.href = url;
    a.download = "nyx-snapshot-" + ((r.summary && r.summary.machine) || "pc") + ".nyx";
    document.body.appendChild(a); a.click(); a.remove(); URL.revokeObjectURL(url);
    pushMsg("bot", { text: snx("done") + "\n\n" + (r.text || "") });
  } catch (e) { tp.remove(); pushMsg("bot", { text: serverHint() }); }
}

function runRestorePick(){
  var inp = document.createElement("input");
  inp.type = "file"; inp.accept = ".nyx,.json,application/json";
  inp.onchange = async function(){
    var f = inp.files && inp.files[0]; if (!f) return;
    var snap; try { snap = JSON.parse(await f.text()); } catch (e) { pushMsg("bot", { text: snx("bad") }); return; }
    var c = active(); if (!c) return;
    pushMsg("user", { text: f.name }); maybeName(c, "restore snapshot");
    var tp = showTyping();
    try {
      var r = await fetch("/api/snapshot/plan", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ snapshot: snap }) }).then(function(x){ return x.json(); });
      tp.remove();
      if (!r || !r.recognized) { pushMsg("bot", { text: ((r && r.error) || snx("bad")) }); return; }
      pushMsg("bot", { text: snx("plan") + "\n\n" + (r.text || ""), actions: [
        { label: snx("restore"), kind: "primary", run: function(){ execRestore(snap); } },
        { label: t("cancel"), kind: "danger", run: function(){ pushMsg("bot", { text: t("execCancel") }); } },
      ] });
    } catch (e) { tp.remove(); pushMsg("bot", { text: serverHint() }); }
  };
  inp.click();
...
async function execRestore(snap){
  var tp = showTyping();
  try {
    var r = await fetch("/api/snapshot/apply", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ snapshot: snap, confirm: true }) }).then(function(x){ return x.json(); });
    tp.remove();
    if (!r || r.type === "error") { pushMsg("bot", { text: ((r && r.error) || t("failed")) }); return; }
    if (r.type === "proposal") { pushMsg("bot", { text: snx("plan") }); return; }
    var lines = (r.results || []).map(function(x){ var ok = (x.ok || x.code === 0); return (ok ? "✓" : "✕") + " " + x.kind; }).join("\n");
    pushMsg("bot", { text: snx("restored") + "\n\n" + (lines || "") });
  } catch (e) { tp.remove(); pushMsg("bot", { text: serverHint() }); }
}

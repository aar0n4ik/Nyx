...
createServer(async (req, res) => {
  try {
    ...
    // --- Vault: DPAPI (Electron safeStorage) — запечатывание ключа для on-device шифрования ---
    if (req.method === "GET" && path === "/api/vault/status") {
      try {
        const el = await import("electron")
        const safe = el.safeStorage || (el.default && el.default.safeStorage)
        const ok = !!(safe && safe.isEncryptionAvailable && safe.isEncryptionAvailable())
        return json(res, 200, { dpapi: ok })
      } catch (e) { return json(res, 200, { dpapi: false }) }
    }
    if (req.method === "POST" && path === "/api/vault/seal") {
      try {
        const { secret } = await readBody(req)
        if (!secret) return json(res, 400, { error: "secret required" })
        const el = await import("electron")
        const safe = el.safeStorage || (el.default && el.default.safeStorage)
        if (!safe || !safe.isEncryptionAvailable()) return json(res, 200, { dpapi: false })
        const sealed = safe.encryptString(String(secret)).toString("base64")
        return json(res, 200, { sealed })
      } catch (e) { return json(res, 200, { error: String((e && e.message) || e) }) }
    }
    if (req.method === "POST" && path === "/api/vault/unseal") {
      try {
        const { sealed } = await readBody(req)
        if (!sealed) return json(res, 400, { error: "sealed required" })
        const el = await import("electron")
        const safe = el.safeStorage || (el.default && el.default.safeStorage)
        if (!safe || !safe.isEncryptionAvailable()) return json(res, 200, { dpapi: false })
        const secret = safe.decryptString(Buffer.from(String(sealed), "base64"))
        return json(res, 200, { secret })
      } catch (e) { return json(res, 200, { error: String((e && e.message) || e) }) }
    }
    // --- Snapshot restore (реальные изменения; только с явным confirm; сначала точка восстановления) ---
    if (req.method === "POST" && path === "/api/snapshot/apply") {
      try {
        const body = await readBody(req)
        const input = body.snapshot || body
        const m = await import("./src/core/snapshot.js")
        const S = m.default || m
        const ident = await S.identify(input)
        if (!ident.recognized) return json(res, 200, { type: "error", error: "Файл не распознан как снимок Nyx" })
        if (body.confirm !== true) return json(res, 200, { type: "proposal", steps: S.plan(ident.snapshot, { select: body.select }) })
        const out = await S.restore(ident.snapshot, { confirm: true, select: body.select, dataDir: body.dataDir })
        if (out && out.type === "done") import("./src/accounts/ledger.js").then((L) => L.record("task", { kind: "restore" })).catch(() => {})
        return json(res, 200, out)
      } catch (e) { return json(res, 200, { type: "error", error: String((e && e.message) || e) }) }
    }
